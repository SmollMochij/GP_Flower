...
AnimatedSprite::AnimatedSprite()
	: m_pVertexData(0), m_iFrameWidth(0), m_iFrameHeight(0), m_iCurrentFrame(0), m_iTotalFrames(0), m_fTimeElapsed(0.0f), m_bAnimating(false), m_bLooping(false), m_frameDuration(1.0f), totalTime(0.0f)
{
}
AnimatedSprite::~AnimatedSprite()
{
...
void AnimatedSprite::SetupFrames(int fixedFrameWidth, int fixedFrameHeight)
{
	...
	for (int h = 0; h < totalFramesHigh; ++h)
	{
		for (int w = 0; w < totalFramesWide; ++w)
		{
			...
			float quad[] =
			{
				-0.5f,  0.5f, 0, uOffset, vOffset + vFrameHeight,
				 0.5f,  0.5f, 0, uFrameWidth + uOffset, vOffset + vFrameHeight,
				 0.5f, -0.5f, 0, uFrameWidth + uOffset, vOffset,
				-0.5f, -0.5f, 0, uOffset, vOffset,
			};	
...
void AnimatedSprite::Process(float deltaTime)
{
	...
	if (m_bAnimating)
	{
		m_fTimeElapsed += deltaTime; // Count how long the current
		...
		{ // swap to the next frame
			...
			{
				if (m_bLooping)
				{ // Loop at the end of the animation
					Restart();
				}
				else
				{
